...
class JsonRestApi(devicecomm):
    ...
    def get_all_event(self, master: int = None, port: int = None) -> any:
        ...
        url = f"{self._url_protocol}{self._hostname}{self._url_prefix}/gateway/events?&masterNumber={master}&portNumber={port}"
    ...
    def post_reboot(self, master: int = None, port: int = None) -> None:
        if master is None:
            master = self._master
        if port is None:
            port = self._port
        ...
        url = f"{self._url_protocol}{self._hostname}{self._url_prefix}/gateway/reboot"

        logging.debug("Request URI: " + url)

        respone = requests.post(url, auth=self._basic_auth)
